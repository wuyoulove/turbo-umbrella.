...
$(function(){
	var $facebox=$("<div class='showFace'></div>");
	for(var key in userFaces){
		var $img=$("<img src='img/faces/"+key+"' alt='"+userFaces[key]+"' />");
		$facebox.append($img);
	}
	$(".insertFace").append($facebox).hover(function(){
		$facebox.show();
	},function(){
		$facebox.hide();
	});
	...
	$facebox.on("click","img",function(){
		$("#msgTxt").text($("#msgTxt").text()+"["+$(this).attr("alt")+"]");
	})
	var $friendbox=$("<div class='showfriend'></div>");
	for(var key in friendsList){
		var $p=$("<p></p>");
		$p.html("@"+friendsList[key]);
		$friendbox.append($p);
	}
	$(".atSome").append($friendbox).hover(function(){
		$(".showfriend").show();
	},function(){
		$(".showfriend").hide();
	});
	$(".showfriend").hide();
	$(".showfriend").on("click","p",function(){
		$("#msgTxt").text($("#msgTxt").text()+"<"+$(this).html()+">");
	})
})